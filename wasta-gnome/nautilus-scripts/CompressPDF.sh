...
if [[ "x$NAUTILUS_SCRIPT_SELECTED_FILE_PATHS" = "x"  -o  "$#" = "0" ]]; then # we double check. Remove the first part if you plan to manually invoke the script
...
INPUT=("$@")
N=("$#")
if [ "${#INPUT[@]}" != "$N" ]; then # comparing the number of arguments the script is given with what it can count
...
if [ $# -eq 1 ]; then
	...
	case "$filename_suffix" in
		*/*)
			$ZENITY --error --title="Compress PDF $VERSION"
			exit 1
		;;
	esac # Check if the specified suffix is legal (we use 'case' instead of 'if' to directly use asterisk * globbing -- and avoid [[...]] for portability)
fi
